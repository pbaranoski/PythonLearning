...
####################################
# pre-geocoding work 
# --> replace with calls to DBMS
# --> create csv file from DBMS
#
# For each input file --> process
# Do we need a FID? Or is that just to join output results back to input?
# !! take input csv file --> DF --> back to csv file --> csv file input to arc process
####################################
...
############################
# feature class needed
# !! How to create feature_class_name instead of using in_csv filename (no path)?
#    Maybe use some literal + timestamp
############################
...
########################################
# Replace read_csv with pandas.read_sql
########################################
            f = io.StringIO()
...
###########################################
# Convert to df to csv file as input to arc 
###########################################
...
#            csvf1 = csvf1[:-4]    ## no extention
            write_log("Message::Started[{}]".format(csvf1))
            #print(("Message::Started[{}]".format(csvf1)))


#################################################################
# Call arc geocoding module
# input --> input_csv
# Exception logic should change. We are not renaming input file.
#################################################################
            try:
...
#########################################
# 1) Convert out_feature_class to csv 
# 2) convert csv to DF
# 3) write csv to DBMS
#########################################
...
# Also, we need to take output file and update database.
# Can we do this using a Data Frame?
# What tables need inserting and updating?
# !!! We cam take the out_csv (merged), and convert to DF
# then write to database using DF.
###########################################################
